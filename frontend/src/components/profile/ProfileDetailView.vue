          ...
          <div v-if="showModal" @click.self="showModal = false" style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0, 0, 0, 0.5);">
            <div style="background: white; padding: 20px; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);">
              ...
              <div style="display: flex; flex-wrap: wrap; gap: 10px;">
                <!-- 이미지 목록 -->
                <img v-for="(image, index) in images" :key="index" :src="image" @click="selectImage(index, user_id)" alt="Option" style="width: 100px; cursor: pointer;">
              </div>
            </div>
          </div>
        </div>
        <div>
            <span>팔로워 수: {{ followers }}명</span>
            <hr>
            <span>팔로잉 수: {{ followings }}명</span>
            <hr>
            <span>닉네임 변경: </span>
            <hr>
            <span>작성한 리뷰 수: </span>
